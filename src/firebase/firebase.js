...
export async function addProperty(address, city, state, zip, unit) {
  try {
    ...
    const db = firebase.firestore();
    db.collection("properties").doc(currentUser.uid).set({
      address: address,
      city: city,
      state: state,
      zip: zip,
...
export async function addTransaction(
  ...
  try {
    ...
    const db = firebase.firestore();
    db.collection("transactions").doc(currentUser.uid).set({
      payment: payment,
...
export async function addTenant(
  ...
  try {
    ...
    const db = firebase.firestore();
    db.collection("tenants").doc(currentUser.uid).set({
      tenant: tenant,
      email: email,
      address: address,
      archived: archived,
  ...
  try {
    ...
    firebase
      ...
      .then(() => {
        firebase
          .auth()
          .currentUser.updateEmail(email.trim().toLowerCase())
          .then(() => {
